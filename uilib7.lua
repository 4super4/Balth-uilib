    ...
    do 
        local dropdown = {} do 
            ...
            dropdown.addOption = function(self, option)
                ...
                optionButton.Position = UDim2.new(0, 0, 0, (#self.options) * 20) -- Adjust position based on option count and height
                optionButton.Parent = self.instances.menu
                optionButton.MouseButton1Click:Connect(function()
                    self:close()
                    self:fireEvent('onSelect', option.value)
                end)
                table.insert(self.options, option) -- Keep track of the option
            end
